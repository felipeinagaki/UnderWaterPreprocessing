...
def run(base_path=None, input_dirname=None, output_dirname=None):
	...
	for i in range(len(files)):
		...
		if os.path.isfile(filepath):
			...
			after_paths.append(os.path.join(out_path, prefix + '_RoWS.' + format_))
			cv2.imwrite(after_paths[-1], sceneRadiance)
			cv2.imwrite(os.path.join(out_path, prefix + '_RoWS_transmission.' + format_), np.uint8(transmission * 255))
